...
def test_create_project_in_mock_mode(monkeypatch):
    ...
    from uuid import uuid4
    pid = str(uuid4())
    p = ProjectCreate(profile_id=pid, name='P', client_name='C', address='A', current_reserve_balance='0')
...
def test_update_project_in_mock_mode(monkeypatch):
    ...
    updated = db.update_project(project_id, profile_id, ProjectUpdate(name='New Name'))
