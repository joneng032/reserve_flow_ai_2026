...
class Database:
    def __init__(self):
        ...
        strict_init = str(os.getenv("DB_STRICT_INIT", "")).lower() in ("1", "true", "yes")

        if not self.supabase_url or not self.supabase_key:
            if strict_init:
                raise ValueError("SUPABASE_URL and SUPABASE_ANON_KEY must be set when DB_STRICT_INIT is true")
            # Non-strict mode: allow mock client for local development & tests.
            print("SUPABASE_URL or SUPABASE_ANON_KEY not set; using mock database client")
            self.client = None
            return
        ...
        try:
            ...
            self.client = None
        except Exception as e:  # Broad catch required to normalize third-party DB client errors into DatabaseError; noqa: S110
    ...
    def get_profile(self, profile_id: str) -> Optional[Profile]:
        ...
        try:
            ...
            if response.data:
                ...
                data = dict(response.data[0]) if isinstance(response.data[0], dict) else dict(response.data[0].__dict__)
                if 'name' not in data:
                    # Some legacy exports use 'username' or only have an email.
                    data['name'] = data.get('username') or data.get('email') or 'Unknown'
                    try:
                        ...
                        return Profile.model_validate({
                            ...
                            'name': data.get('name') or data.get('username') or data.get('email') or 'Unknown',
                        })
            return None
    ...
    def update_profile(self, profile_id: str, profile_data: ProfileUpdate) -> Optional[Profile]:
    ...
    # Project operations
    def get_projects(self, profile_id: str, skip: int = 0, limit: int = 100) -> List[ProjectWithDetails]:
        if not self.client:
        ...
        try:
            response = self.client.table('projects').select(
                ...
                *,
                ...
                '''
            ).eq('profile_id', profile_id).range(skip, skip + limit - 1).execute()

            projects = []
            for item in response.data:
                # Calculate totals
                components_count = len(item.get('components', []))
                total_value = sum(float(c.get('base_cost', 0) or 0) for c in item.get('components', []))

                project = ProjectWithDetails(
                    **{k: v for k, v in item.items() if k not in ['categories', 'components']},
    ...
    def get_project(self, project_id: str, profile_id: str) -> Optional[ProjectWithDetails]:
    ...
    def create_project(self, project_data: ProjectCreate) -> Optional[Project]:
        if not self.client:
            ...
            return Project(
                ...
                current_reserve_balance=project_data.current_reserve_balance or Decimal('0'),
    ...
    def update_project(self, project_id: str, profile_id: str, project_data: ProjectUpdate) -> Optional[Project]:
        ...
        if not self.client:
            ...
            return types.SimpleNamespace(
                ...
                current_reserve_balance=data.get('current_reserve_balance', Decimal('0')),
        ...
        try:
            data = project_data.model_dump(exclude_unset=True)
            response = self.client.table('projects').update(data).eq('id', project_id).eq('profile_id', profile_id).execute()
            if response.data:
                return Project(**response.data[0])
            return None
        except (ValueError, TypeError, KeyError, AttributeError) as e:
    ...
    def delete_project(self, project_id: str, profile_id: str) -> bool:
        ...
        try:
            response = self.client.table('projects').delete().eq('id', project_id).eq('profile_id', profile_id).execute()
    ...
    def get_project_components(self, project_id: str, profile_id: str, category: Optional[str] = None, tag: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[ComponentWithTags]:
        ...
        try:
            ...
            project_check = self.client.table('projects').select('id').eq('id', project_id).eq('profile_id', profile_id).execute()
    ...
    def create_component(self, component_data: ComponentCreate, profile_id: str) -> Optional[Component]:
        if not self.client:
            # Mock implementation
            from uuid import uuid4
            return Component(
        ...
        try:
            ...
            project_check = self.client.table('projects').select('id').eq('id', component_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"component_name": component.name, "category": component.category}
                )
    ...
    def update_component(self, component_id: str, component_data: ComponentUpdate, profile_id: str) -> Optional[Component]:
        ...
        try:
            ...
            component_check = self.client.table('components').select(
                'project_id, projects!inner(profile_id)'
            ).eq('id', component_id).eq('projects.profile_id', profile_id).execute()

            if not component_check.data:
                return None

            data = component_data.model_dump(exclude_unset=True)
            response = self.client.table('components').update(data).eq('id', component_id).execute()
            if response.data:
                component = Component(**response.data[0])
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"component_name": component.name, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    # Category operations
    def get_project_categories(self, project_id: str, profile_id: str) -> List[Category]:
        ...
        try:
            ...
            response = self.client.table('categories').select('*').eq('project_id', project_id).execute()
    ...
    def create_category(self, category_data: CategoryCreate, profile_id: str) -> Optional[Category]:
        if not self.client:
            # Mock implementation
            from uuid import uuid4
            return Category(
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', category_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
    ...
    def update_category(self, category_id: str, category_data: CategoryUpdate, profile_id: str) -> Optional[Category]:
        try:
            # First verify category ownership through project
            category_check = self.client.table('categories').select(
                'project_id, projects!inner(profile_id)'
            ).eq('id', category_id).eq('projects.profile_id', profile_id).execute()
    ...
    def get_cost_analysis(self, project_id: str, profile_id: str) -> CostAnalysis:
        ...
        if not self.client:
            return CostAnalysis(total_components=0, total_value=0, average_cost=0, categories_breakdown={})
        ...
        try:
            ...
            components_response = self.client.table('components').select('*').eq('project_id', project_id).execute()
            ...
            for component in components:
                ...
                categories_breakdown[category]['total_value'] += float(component.get('base_cost', 0) or 0)
            ...
            for category in categories_breakdown:
                ...
                categories_breakdown[category]['average_cost'] = total / count if count > 0 else 0
    ...
    def get_reserve_analysis(self, project_id: str, profile_id: str) -> ReserveAnalysis:
        ...
        try:
            # Get project info
            project_response = self.client.table('projects').select('*').eq('id', project_id).eq('profile_id', profile_id).execute()
            ...
            percent_funded = (reserve_balance / total_liability * 100) if total_liability > 0 else 0
            funding_gap = max(0, total_liability - reserve_balance)

            recommendations = []
            if percent_funded < 70:
                recommendations.append("Reserve balance is below recommended 70% funding level")
            if funding_gap > 0:
                recommendations.append(f"Consider funding ${funding_gap:,.2f} to reach 100% funding")
            if cost_analysis.total_components == 0:
                recommendations.append("Add components to your project for accurate reserve analysis")
    ...
    def set_project_metro(self, project_id: str, metro_data: ProjectMetroSettingCreate, profile_id: str) -> Optional[ProjectMetroSetting]:
        ...
        try:
            ...
            existing = self.client.table('project_metro_settings').select('*').eq('project_id', project_id).execute()
            ...
            if existing.data:
                # Update existing
                response = self.client.table('project_metro_settings').update(data).eq('project_id', project_id).execute()
            else:
                # Create new
                response = self.client.table('project_metro_settings').insert(data).execute()
    ...
    def get_project_metro(self, project_id: str, profile_id: str) -> Optional[ProjectMetroSetting]:
        # Mock-mode: return None when client is not available
        if not self.client:
            return None
        ...
        try:
            ...
            response = self.client.table('project_metro_settings').select('*').eq('project_id', project_id).execute()
            if response.data:
                return ProjectMetroSetting(**response.data[0])
            return None
        except (ValueError, TypeError, KeyError, AttributeError) as e:
    ...
    # Component catalog
    def get_component_catalog(self, category: Optional[str] = None) -> List[ComponentCatalog]:
        # Mock mode: return empty list when client is not available
        if not self.client:
            return []
        ...
        try:
            ...
            return []
        except Exception as e:  # Broad catch required to normalize third-party DB client errors into DatabaseError; noqa: S110  # type: ignore[reportBroadExceptionCaught]
    ...
    def get_project_audit_logs(self, project_id: str, profile_id: str, entity_type: Optional[str] = None, skip: int = 0, limit: int = 50) -> List[AuditLog]:
        ...
        try:
            ...
            query = self.client.table('audit_logs').select('*').eq('project_id', project_id)
            if entity_type:
    ...
    # Meeting operations
    def create_meeting(self, meeting_data: MeetingCreate, profile_id: str) -> Optional[Meeting]:
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', meeting_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"meeting_title": meeting.title, "meeting_type": meeting.meeting_type}
                )
                self.create_audit_log(audit_data)
    ...
    def get_project_meetings(self, project_id: str, profile_id: str, meeting_type: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[Meeting]:
        ...
        try:
            ...
            query = self.client.table('meetings').select('*').eq('project_id', project_id)
            if meeting_type:
                query = query.eq('meeting_type', meeting_type)

            response = query.order('meeting_date', desc=True).range(skip, skip + limit - 1).execute()
    ...
    def get_meeting(self, meeting_id: str, profile_id: str) -> Optional[Meeting]:
        ...
        try:
            ...
            meeting_check = self.client.table('meetings').select(
                '*, projects!inner(profile_id)'
            ).eq('id', meeting_id).eq('projects.profile_id', profile_id).execute()
    ...
    def update_meeting(self, meeting_id: str, meeting_data: MeetingUpdate, profile_id: str) -> Optional[Meeting]:
        ...
        try:
            ...
            data = meeting_data.model_dump(exclude_unset=True)
            response = self.client.table('meetings').update(data).eq('id', meeting_id).execute()
            if response.data:
                meeting = Meeting(**response.data[0])
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"meeting_title": meeting.title, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    def delete_meeting(self, meeting_id: str, profile_id: str) -> bool:
        ...
        try:
            ...
            response = self.client.table('meetings').delete().eq('id', meeting_id).execute()
            if len(response.data) > 0:
                # Create audit log
                audit_data = AuditLogCreate(
    ...
    def create_communication(self, communication_data: CommunicationCreate, profile_id: str) -> Optional[Communication]:
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', communication_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"communication_type": communication.communication_type, "subject": communication.subject}
                )
                self.create_audit_log(audit_data)
    ...
    def get_project_communications(self, project_id: str, profile_id: str, communication_type: Optional[str] = None, status: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[Communication]:
        ...
        try:
            ...
            query = self.client.table('communications').select('*').eq('project_id', project_id)
    ...
    def get_communication(self, communication_id: str, profile_id: str) -> Optional[Communication]:
        ...
        try:
            ...
            communication_check = self.client.table('communications').select(
                '*, projects!inner(profile_id)'
            ).eq('id', communication_id).eq('projects.profile_id', profile_id).execute()
            ...
            return None
        except Exception as e:  # Broad catch required to protect API behavior when DB client raises unexpected errors; noqa: S110  # type: ignore[reportBroadExceptionCaught]
            print(f"Unexpected error getting communication: {e}")
            raise DatabaseError(str(e)) from e

    def update_communication(self, communication_id: str, communication_data: CommunicationUpdate, profile_id: str) -> Optional[Communication]:
        ...
        try:
            ...
            data = communication_data.model_dump(exclude_unset=True)
            response = self.client.table('communications').update(data).eq('id', communication_id).execute()
            if response.data:
                communication = Communication(**response.data[0])
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"communication_type": communication.communication_type, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    def delete_communication(self, communication_id: str, profile_id: str) -> bool:
        ...
        try:
            ...
            response = self.client.table('communications').delete().eq('id', communication_id).execute()
            if len(response.data) > 0:
                # Create audit log
                audit_data = AuditLogCreate(
                    ...
                    action="delete",
                    user_id=profile_id,
                    details={"communication_type": response.data[0].get('communication_type', 'Unknown')}
                )
                self.create_audit_log(audit_data)
    ...
    def create_media_file(self, media_file_data: MediaFileCreate, profile_id: str) -> Optional[MediaFile]:
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', media_file_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"file_name": media_file.file_name, "file_type": media_file.file_type}
                )
                self.create_audit_log(audit_data)
    ...
    def get_project_media_files(self, project_id: str, profile_id: str, file_type: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[MediaFile]:
        ...
        try:
            ...
            query = self.client.table('media_files').select('*').eq('project_id', project_id)
            if file_type:
    ...
    def get_media_file(self, media_file_id: str, profile_id: str) -> Optional[MediaFile]:
        ...
        try:
            ...
            media_file_check = self.client.table('media_files').select(
                '*, projects!inner(profile_id)'
            ).eq('id', media_file_id).eq('projects.profile_id', profile_id).execute()
    ...
    def update_media_file(self, media_file_id: str, media_file_data: MediaFileUpdate, profile_id: str) -> Optional[MediaFile]:
        ...
        try:
            ...
            data = media_file_data.model_dump(exclude_unset=True)
            response = self.client.table('media_files').update(data).eq('id', media_file_id).execute()
            if response.data:
                media_file = MediaFile(**response.data[0])
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"file_name": media_file.file_name, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    def delete_media_file(self, media_file_id: str, profile_id: str) -> bool:
        ...
        try:
            ...
            response = self.client.table('media_files').delete().eq('id', media_file_id).execute()
            if len(response.data) > 0:
                # Create audit log
                audit_data = AuditLogCreate(
    ...
    def create_interview(self, interview_data: InterviewCreate, profile_id: str) -> Optional[Interview]:
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', interview_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"interviewee_name": interview.interviewee_name, "interview_type": interview.interview_type}
                )
                self.create_audit_log(audit_data)
    ...
    def get_project_interviews(self, project_id: str, profile_id: str, interview_type: Optional[str] = None, interview_status: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[Interview]:
        # Mock-mode: return empty list when no DB client is available
        if not self.client:
            return []
        ...
        try:
            ...
            query = self.client.table('interviews').select('*').eq('project_id', project_id)
            if interview_type:
                query = query.eq('interview_type', interview_type)
            if interview_status:
                query = query.eq('status', interview_status)

            response = query.order('scheduled_date', desc=True).range(skip, skip + limit - 1).execute()
    ...
    def get_interview(self, interview_id: str, profile_id: str) -> Optional[Interview]:
        ...
        try:
            ...
            interview_check = self.client.table('interviews').select(
                '*, projects!inner(profile_id)'
            ).eq('id', interview_id).eq('projects.profile_id', profile_id).execute()
    ...
    def update_interview(self, interview_id: str, interview_data: InterviewUpdate, profile_id: str) -> Optional[Interview]:
        ...
        try:
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"interviewee_name": interview.interviewee_name, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
        ...
        try:
            ...
            if len(response.data) > 0:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="delete",
                    user_id=profile_id,
                    details={"interviewee_name": response.data[0].get('interviewee_name', 'Unknown')}
                )
                self.create_audit_log(audit_data)
    ...
    def create_inspection(self, inspection_data: InspectionCreate, profile_id: str) -> Optional[Inspection]:
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', inspection_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"inspection_type": inspection.inspection_type, "location": inspection.location}
                )
                self.create_audit_log(audit_data)
    ...
    def get_project_inspections(self, project_id: str, profile_id: str, inspection_type: Optional[str] = None, inspection_status: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[Inspection]:
        # Mock-mode: return empty list when client is not present
        if not self.client:
            return []
        ...
        try:
            ...
            query = self.client.table('inspections').select('*').eq('project_id', project_id)
            if inspection_type:
    ...
    def get_inspection(self, inspection_id: str, profile_id: str) -> Optional[Inspection]:
        # Mock-mode: return None when the DB client is not available
        if not self.client:
            return None
        ...
        try:
            ...
            inspection_check = self.client.table('inspections').select(
                '*, projects!inner(profile_id)'
            ).eq('id', inspection_id).eq('projects.profile_id', profile_id).execute()
    ...
    def update_inspection(self, inspection_id: str, inspection_data: InspectionUpdate, profile_id: str) -> Optional[Inspection]:
        ...
        try:
            ...
            data = inspection_data.model_dump(exclude_unset=True)
            response = self.client.table('inspections').update(data).eq('id', inspection_id).execute()
            if response.data:
                inspection = Inspection(**response.data[0])
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"inspection_type": inspection.inspection_type, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    def delete_inspection(self, inspection_id: str, profile_id: str) -> bool:
        ...
        try:
            ...
            response = self.client.table('inspections').delete().eq('id', inspection_id).execute()
            if len(response.data) > 0:
                # Create audit log
                audit_data = AuditLogCreate(
                    ...
                    action="delete",
                    user_id=profile_id,
                    details={"inspection_type": response.data[0].get('inspection_type', 'Unknown')}
                )
                self.create_audit_log(audit_data)
    ...
    def create_inspection_item(self, inspection_item_data: InspectionItemCreate, profile_id: str) -> Optional[InspectionItem]:
        ...
        try:
            ...
            inspection_check = self.client.table('inspections').select(
                'project_id, projects!inner(profile_id)'
            ).eq('id', inspection_item_data.inspection_id).eq('projects.profile_id', profile_id).execute()
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"item_name": inspection_item.item_name, "item_type": inspection_item.item_type}
                )
                self.create_audit_log(audit_data)
    ...
    def get_inspection_items(self, inspection_id: str, profile_id: str, item_type: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[InspectionItem]:
        ...
        try:
            ...
            query = self.client.table('inspection_items').select('*').eq('inspection_id', inspection_id)
            if item_type:
    ...
    def get_inspection_item(self, inspection_item_id: str, profile_id: str) -> Optional[InspectionItem]:
        # Mock-mode: return None when DB client is not available
        if not self.client:
            return None
        ...
        try:
            ...
            inspection_item_check = self.client.table('inspection_items').select(
                '*, inspections!inner(project_id, projects!inner(profile_id))'
            ).eq('id', inspection_item_id).eq('inspections.projects.profile_id', profile_id).execute()
    ...
    def update_inspection_item(self, inspection_item_id: str, inspection_item_data: InspectionItemUpdate, profile_id: str) -> Optional[InspectionItem]:
        ...
        try:
            ...
            data = inspection_item_data.model_dump(exclude_unset=True)
            response = self.client.table('inspection_items').update(data).eq('id', inspection_item_id).execute()
            if response.data:
                inspection_item = InspectionItem(**response.data[0])

                # Create audit log
                audit_data = AuditLogCreate(
                    project_id=inspection_item_check.data[0]['inspections']['project_id'],
                    entity_type="inspection_item",
                    entity_id=inspection_item_id,
                    action="update",
                    user_id=profile_id,
                    details={"item_name": inspection_item.item_name, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    def delete_inspection_item(self, inspection_item_id: str, profile_id: str) -> bool:
        ...
        try:
            ...
            response = self.client.table('inspection_items').delete().eq('id', inspection_item_id).execute()
            if len(response.data) > 0:
                # Create audit log
                audit_data = AuditLogCreate(
    ...
    # Evidence operations
    def create_evidence(self, evidence_data: EvidenceCreate, profile_id: str) -> Optional[Evidence]:
        ...
        try:
            # Verify project ownership
            project_check = self.client.table('projects').select('id').eq('id', evidence_data.project_id).eq('profile_id', profile_id).execute()
            if not project_check.data:
                return None
            ...
            if response.data:
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="create",
                    user_id=profile_id,
                    details={"evidence_type": evidence.evidence_type, "file_name": evidence.file_name}
                )
                self.create_audit_log(audit_data)
            ...
            return None
        except Exception as e:  # Unexpected DB/client error — wrap and propagate; noqa: S110  # type: ignore[reportBroadExceptionCaught]
            print(f"Unexpected error creating evidence: {e}")
            raise DatabaseError(str(e)) from e

    def get_project_evidence(self, project_id: str, profile_id: str, evidence_type: Optional[str] = None, skip: int = 0, limit: int = 100) -> List[Evidence]:
        # Mock-mode: return empty list when client is not available
        if not self.client:
            return []
        ...
        try:
            ...
            query = self.client.table('evidence').select('*').eq('project_id', project_id)
            if evidence_type:
                query = query.eq('evidence_type', evidence_type)

            response = query.order('timestamp', desc=True).range(skip, skip + limit - 1).execute()
            return [Evidence(**item) for item in response.data]
        except (ValueError, TypeError, KeyError, AttributeError) as e:
            print(f"Data error getting evidence: {e}")
            return []
        except Exception as e:  # Unexpected DB/client error — wrap and propagate; noqa: S110
    ...
    def get_evidence(self, evidence_id: str, profile_id: str) -> Optional[Evidence]:
        ...
        try:
            ...
            evidence_check = self.client.table('evidence').select(
                '*, projects!inner(profile_id)'
            ).eq('id', evidence_id).eq('projects.profile_id', profile_id).execute()
    ...
    def update_evidence(self, evidence_id: str, evidence_data: EvidenceUpdate, profile_id: str) -> Optional[Evidence]:
        ...
        try:
            ...
            data = evidence_data.model_dump(exclude_unset=True)
            response = self.client.table('evidence').update(data).eq('id', evidence_id).execute()
            if response.data:
                evidence = Evidence(**response.data[0])
                ...
                audit_data = AuditLogCreate(
                    ...
                    action="update",
                    user_id=profile_id,
                    details={"evidence_type": evidence.evidence_type, "updated_fields": list(data.keys())}
                )
                self.create_audit_log(audit_data)
    ...
    def delete_evidence(self, evidence_id: str, profile_id: str) -> bool:
        ...
        try:
            ...
            response = self.client.table('evidence').delete().eq('id', evidence_id).execute()
            if len(response.data) > 0:
                # Create audit log
                audit_data = AuditLogCreate(
                    ...
                    action="delete",
                    user_id=profile_id,
                    details={"evidence_type": response.data[0].get('evidence_type', 'Unknown')}
                )
                self.create_audit_log(audit_data)
