...
class SimpleJWTStrategy(ITokenStrategy):
    ...
    def create_token(self, data: Dict[str, Any], expires_delta: Optional[timedelta] = None) -> str:
        ...
        signing_input = f"{self._b64u_encode(header_b)}.{self._b64u_encode(payload_b)}".encode("ascii")

        sig = hmac.new(self.settings.secret_key.encode("utf-8"), signing_input, hashlib.sha256).digest()
    ...
    def verify_token(self, token: str) -> Dict[str, Any]:
        ...
        expected_sig = hmac.new(self.settings.secret_key.encode("utf-8"), signing_input, hashlib.sha256).digest()
...
class TokenService:
    ...
    def __init__(self, strategy: ITokenStrategy = None):
        ...
        use_simple = str(os.getenv("USE_SIMPLE_JWT", "")).lower() in ("1", "true", "yes")
